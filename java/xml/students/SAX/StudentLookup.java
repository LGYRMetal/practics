...
public class StudentLookup extends DefaultHandler {
    ...
    public void startElement(String uri, String localName, String qName,
        ...
        // 判断栈是否为空，如果不为空，说明已经找到了匹配的学生，于是将其子
        // 元素的名字入栈
        if (!tagsStack.empty()) {
            tagsStack.push(qName);
        } else {
            // 判断是否是<student>元素，如果是，则取出该元素的所有属性进行
            // 比对。如果找到匹配的学生，则将该元素的名字入栈。
    ...
    public void characters(char[] ch, int start, int length)
        ...
        if (!tagsStack.empty()) {
            // 得到栈顶的对象，判断是学生名字还是年龄，然后将字符数据保存到
            // name或age变量中
            // JDK1.5以下版本，调用为
    ...
    public void endElement(String uri, String localName, String qName)
        throws SAXException {
        if (!tagsStack.empty()) {
            // 弹出栈顶对象。JDK1.5一下版本，调用为
            // String tag = (String) tagsStack.pop();
            String tag = tagsStack.pop();

            // 判断移除栈顶对象后，栈是否为空，如果为空，并且移除的栈顶对象
            // 是<student>元素的名字，则说明所有学生信息都已获取完毕，于是
            // 打印输出找到的学生信息，然后抛出SAXException异常，终止SAX
            // 解析器的解析过程
            if (tagsStack.empty() && "student".equals(tag)) {
                System.out.println("name: " + name);
                System.out.println("age: " + age);
                throw new SAXException("找到符合的学生");
            }
        }
    }
